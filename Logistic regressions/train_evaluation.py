...
def train(**keywords):
    ...
    **keywords:
    ...
    for epoch in range(keywords['num_epochs']):
        total_batch_training = int(keywords['data']['train/image'].shape[0] / keywords['batch_size'])
        ...
        for batch_num in range(total_batch_training):
            ...
            train_batch_data, train_batch_label = keywords['data']['train/image'][start_idx:end_idx], keywords['data']['train/label'][
                                                                                        start_idx:end_idx]
        ...
        if keywords['online_test']:
            ...
            test_accuracy_epoch, test_summaries = keywords['sess'].run([keywords['tensors']['accuracy'], keywords['tensors']['summary_test_op']],
                                                           feed_dict={keywords['tensors']['image_place']: keywords['data']['test/image'],
                                                                      keywords['tensors'][
                                                                          'label_place']: keywords['data']['test/label'],
                                                                      keywords['tensors'][
                                                                          'dropout_param']: 1.})
...
def evaluation(**keywords):
        ...
        test_accuracy = 100 * keywords['sess'].run(keywords['tensors']['accuracy'], feed_dict={keywords['tensors']['image_place']: keywords['data']['test/image'],
                                                                                               keywords['tensors'][
                                                                           'label_place']: keywords['data']['test/label'],
                                                                                               keywords['tensors'][
                                                                           'dropout_param']: 1.})
